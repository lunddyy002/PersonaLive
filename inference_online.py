...
class App:
    ...
    def init_app(self):
        ...
        async def push_results_to_client(user_id: uuid.UUID, websocket: WebSocket):
            ...
            try:
                while True:
                    frame = await self.conn_manager.get_latest_frame(user_id)
                    if frame is None:
                    ...
                    if now - last_log_time > 5.0:
                        try:
                            qsize = await self.conn_manager.get_output_queue_size(user_id)
                            fps = sent_frames / (now - last_log_time) if last_log_time > 0 else 0.0
                            logging.info(f"[stream] q={qsize} fps={fps:.1f}")
                        except Exception:
                            pass
                        last_log_time = now
                        sent_frames = 0
