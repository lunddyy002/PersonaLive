...
class ConnectionManager:
    def __init__(self):
        ...
        self._output_queue_maxsize = int(os.environ.get("OUTPUT_QUEUE_MAXSIZE", "2"))
    ...
    async def connect(
        ...
        self.active_connections[user_id] = {
            ...
            "output_queue": asyncio.Queue(maxsize=self._output_queue_maxsize),
        }
    ...
    async def get_latest_frame(self, user_id: UUID) -> bytes:
        session = self.active_connections.get(user_id)
        if session:
            queue = session["output_queue"]
